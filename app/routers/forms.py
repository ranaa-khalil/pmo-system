...
def create_form_instance(
    ...
    template = db.query(FormTemplate).filter(FormTemplate.id == form.template_id).first()
    if not template:
...
def list_project_forms(
    ...
    instances = db.query(FormInstance).filter(FormInstance.project_id == project_id).all()
    for inst in instances:
        inst.template = db.query(FormTemplate).filter(FormTemplate.id == inst.template_id).first()
    return instances
...
def get_form_instance(
    ...
    instance = db.query(FormInstance).filter(FormInstance.id == form_id).first()
    if not instance:
        raise HTTPException(status_code=404, detail="Form instance not found")
    instance.template = db.query(FormTemplate).filter(FormTemplate.id == instance.template_id).first()
    return instance

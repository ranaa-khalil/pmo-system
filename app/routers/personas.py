...
def create_persona(project_id: int, persona: PersonaCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db_persona = UserPersona(project_id=project_id, **persona.model_dump(exclude={"project_id"}))
    db.add(db_persona)
    db.commit()
...
def update_persona(project_id: int, persona_id: int, persona: PersonaUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_persona = db.query(UserPersona).filter(UserPersona.id == persona_id, UserPersona.project_id == project_id).first()
    if not db_persona:
...
def delete_persona(project_id: int, persona_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_persona = db.query(UserPersona).filter(UserPersona.id == persona_id, UserPersona.project_id == project_id).first()
    if not db_persona:
        raise HTTPException(status_code=404, detail="Persona not found")
    if not can_manage_project(current_user, db_persona.project, db):
...
def create_test_account(project_id: int, account: TestAccountCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    project = db.query(Project).filter(Project.id == project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    if not can_manage_project(current_user, project, db):
        raise HTTPException(status_code=403, detail="Not authorized to manage this project")
    db_account = ProjectTestAccount(project_id=project_id, **account.model_dump(exclude={"project_id"}))
    db.add(db_account)
    db.commit()
...
def update_test_account(project_id: int, account_id: int, account: TestAccountUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_account = db.query(ProjectTestAccount).filter(ProjectTestAccount.id == account_id, ProjectTestAccount.project_id == project_id).first()
    if not db_account:
...
def delete_test_account(project_id: int, account_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_account = db.query(ProjectTestAccount).filter(ProjectTestAccount.id == account_id, ProjectTestAccount.project_id == project_id).first()
    if not db_account:
        raise HTTPException(status_code=404, detail="Test account not found")
    if not can_manage_project(current_user, db_account.project, db):
